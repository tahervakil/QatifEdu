  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container,Bundle savedInstanceState) {
	...
	btn_Favorite.setOnClickListener(new OnClickListener() {
			public void onClick(View v) { 
				try{Object obj[] = (Object[])btn_Favorite.getTag();
		...
		btn_Share.setOnClickListener(new OnClickListener() {	
				...
				public void onClick(View v) {
				    ...
				    sharingIntent.putExtra(android.content.Intent.EXTRA_TEXT, Ads.getTitle()+ "\n" + "http://qatifedu.com/cmsQatifedu/frmAds.aspx?id="
...
//}
    //else 
		...
		if (Utility.isConnected(act)) 
			new Fetch_AdsData().execute(new String[]{id});
			else
  ...
  private Cursor getAdsContentsCursorByID(String ID) {
		Cursor crsr = null;
		try {
			String sql =String.format(Locale.US, "select %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s from %s where %s=%s  " +
					" AND Deleted='0' " , Constants.ID, Constants.SECTIONID, Constants.TITLE, Constants.TEXT,
					 Constants.IMAGE, Constants.STARTDATE, Constants.ENDDATE, Constants.LASTCHANGE, Constants.VIEWD, Constants.STATUS, Constants.ISFIRST, Constants.ENDDATESTRING, Constants.ADS_TABLE, Constants.ID,ID);
  ...
  private boolean updateAdsViewedStatus(String id, int Adstatus) {
		...
		try {
			String sql = String.format(Locale.US, "UPDATE %s SET %s = %s WHERE (%s = %s)", Constants.ADS_TABLE, Constants.VIEWD, Adstatus, Constants.ID, id);
			db.execSQL(sql);
			result = true;
		} catch (Exception ex) {
			ex.getMessage();
 ...
 /* 
  public static void CheckPosition(int position){
	 ...
	 private Cursor getBannersCursor() {
			Cursor crsr = null;
			try {
				String sql = String.format(Locale.US, "select %s,%s,%s,%s from %s order by %s DESC ", Constants.ID, Constants.TITLE, Constants.DETIALS,
						Constants.IMAGE, Constants.BANNER_TABLE," LastChange ");
				crsr = db.rawQuery(sql, null);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
				...
				image.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						if (alBannersMainDataList != null && alBannersMainDataList.get(pos) != null ) {
			    		    ...
			    		    ft.replace(R.id.frame_container, Constants.MFragmentStack.push(fragment),"banner").commit();}
					}
				});
	            
				if(banner!=null && banner.getImage()!=null){
				imageLoader.displayImage("http://qatifedu.com/cmsQatifedu/Upload/Banner/"
						+ banner.getImage(), image, options, new SimpleImageLoadingListener() {
	   	    	@Override
	 ...
	 public  void setData(int iPos){
		 ...
		 name =Ads.getTitle();
		 ((TextView) act.getSupportActionBar().getCustomView().findViewById(R.id.tvTitle)).setText(getString(R.string.details));	
		 Ads_List.Pos=iPos;
		 updateAdsViewedStatus(Ads.getId(),1);
		...
		// webView.getSettings().setUseWideViewPort(true);
		 ...
		 if (Utility.isConnected(act)){
			...
			Utility.showToast(act, act.getString(R.string.alert_need_internet_connection));
		 ((TextView) myFragmentView.findViewById(R.id.tv_title)).setText(Ads.getTitle());
		 ((TextView) myFragmentView.findViewById(R.id.tv_date)).setText(act.getString(R.string.date) + " :" + Ads.getStartDate());
		 //CheckPosition(iPos);
		 IsFavorite();
	  }
	 ...
	 public  class ViewCountLoader extends AsyncTask<Void, Void, String>
		{	
			...
			{   
				if (Utility.isConnected(act)) {
					noofViews = new FetchApiResponse().getJsonValue("http://qatifedu.com/cmsQatifedu/Services/frmViews.aspx?id="+Ads.getId()+"&type=2&addOne=1");
			...
			{   
				try{
					if(Ads_DetailsFrag.isAdded()){
					 }((TextView) myFragmentView.findViewById(R.id.tv_views)).setText(act.getString(R.string.views)+ " : " + v );}catch(Exception ex){}}}
