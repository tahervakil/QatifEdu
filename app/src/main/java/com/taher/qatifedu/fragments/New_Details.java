...
public class New_Details extends /*Sherlock*/Fragment
 {
	...
	btn_Favorite.setOnClickListener(new OnClickListener() {
		...
		btn_Share.setOnClickListener(new OnClickListener() {	
				...
				public void onClick(View v) {
				    ...
				    sharingIntent.putExtra(android.content.Intent.EXTRA_TEXT, news.getTitle()+ "\n" + "http://qatifedu.com/cmsQatifedu/frmNews.aspx?id="
  ...
  private boolean updateNewsViewedStatus(String id, int newStatus) {
		...
		try {
			String sql = String.format(Locale.US, "UPDATE %s SET %s = %s WHERE (%s = %s)", Constants.NEWS_TABLE, Constants.VIEWD, newStatus, Constants.ID, id);
			db.execSQL(sql);
			result = true;
		} catch (Exception ex) {
			ex.getMessage();
			...
			image.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if (alBannersMainDataList != null && alBannersMainDataList.get(pos) != null ) {
		    		    ...
		    		    ft.replace(R.id.frame_container, Constants.NFragmentStack.push(fragment),"banner").commit();}
  ...
  public void CheckPosition(int position){
	 ...
	 public void setData(int iPos){
		...
		// webView.getSettings().setUseWideViewPort(true);
		 ...
		 ((TextView) myFragmentView.findViewById(R.id.tv_date)).setText(getString(R.string.date) + " :" + news.getStartDate());
		 CheckPosition(iPos);
		 IsFavorite();
	  }
	 ...
	 public class ViewCountLoader extends AsyncTask<Void, Void, String>
		{	
			...
			{   
				if (Utility.isConnected(act)) {
					noofViews = new FetchApiResponse().getJsonValue("http://qatifedu.com/cmsQatifedu/Services/frmViews.aspx?id="+news.getId()+"&type=1&addOne=1");
			...
			{   try{
				if(isAdded()){
				((TextView) myFragmentView.findViewById(R.id.tv_views)).setText(getString(R.string.views)+ " : " + v );}}
			catch(Exception ex){}}}
