...
public class ContentUpdater {
	...
	public synchronized String getChangeDate(String strTableName) {
		...
		try {
			...
			Cursor cursor = db.rawQuery("select "+ Constants.CHANGEDATE+" from " + Constants.LATESTCLIENTUPDATE_TABLE + " where " + Constants.TABLENAME+ "='" + strTableName+"'", null);
			if (cursor == null)
				   return strChangeDate;

				try{
				   if (cursor.moveToFirst()) 
					   strChangeDate=cursor.getString(0); 
				}catch(Exception ex) { 
				   cursor.close();
				}
			return strChangeDate;
	...
	public synchronized void updateLatestClientUpdate(String strChangeDate, String strTableName) {
		...
		try {
			String sql = "Update " + Constants.LATESTCLIENTUPDATE_TABLE +" set " + Constants.CHANGEDATE+" = " + strChangeDate +" WHERE " + Constants.TABLENAME+ "='" + strTableName+"'";
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	private void saveContentsForGeneral(More_Entity c) {
		try {
				...
				if (c.getName() != null && !c.getName().equals(""))
					vals.put(Constants.NAME, c.getName());
	...
	private void saveContentsForSections(Section_Entity c) {
		try {
				...
				vals.put(Constants.ID, c.getId());
				if (c.getName() != null && !c.getName().equals(""))
					vals.put(Constants.TITLE, c.getName());
				if (c.getLogo() != null && !c.getLogo().equals(""))
					vals.put(Constants.LOGO, c.getLogo());
				if (c.getLastChange() != null && !c.getLastChange().equals(""))
					vals.put(Constants.LASTCHANGE, c.getLastChange());
	...
	public int getSectionStatus (String strsection) {
        int Status=1;
		try {
			String sql = "Select"+ Constants.STATUS+ " FROM " + Constants.SECTIONS_TABLE + " Where  "+ Constants.ID+" ="+strsection;
	...
	private ArrayList<Section_Entity>  downloadRecentContentsForSections(String strChangeDate) throws IOException, ParserConfigurationException, SAXException {
		return new ContentParser(context.getString(R.string.sections_url) + strChangeDate).getSectionContents();
	...
	public int getCompanyStatus (String strcompany) {
        int Status=1;
		try {
			String sql = "Select"+ Constants.STATUS+ " FROM " + Constants.COMPANIES_TABLE + " Where  "+ Constants.ID+" ="+strcompany;
			Cursor crsr =db.rawQuery(sql, null);
			crsr.moveToNext();
			Status= crsr.getInt(0);
		} catch (Exception ex) {
	...
	private ArrayList<Company_Entity>  downloadRecentContentsForCompanies(String strChangeDate) throws IOException, ParserConfigurationException, SAXException {
		return new ContentParser(context.getString(R.string.companies_url) + strChangeDate).getCompaniesContents();
	...
	public synchronized void deleteOldContentsForBanner(String bannerId) {
		...
		try {
			String sql = "DELETE FROM " + Constants.BANNER_TABLE + " WHERE " + Constants.ID+ "=" + bannerId;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	private void saveContentsForNews(News_Entity c) {
		try {
				...
				if (c.getStatus() != 0)
					vals.put(Constants.STATUS, c.getStatus());
				if (c.getTitle() != null && !c.getTitle().equals(""))
					vals.put(Constants.TITLE, c.getTitle());
				if (c.getText() != null && !c.getText().equals(""))
					vals.put(Constants.TEXT, c.getText());
				if (c.getImage() != null && !c.getImage().equals(""))
					vals.put(Constants.IMAGE, c.getImage());
				...
				if (c.getIsFirst() != 0)
					vals.put(Constants.ISFIRST, c.getIsFirst());
				if (c.getLastChange() != null && !c.getLastChange().equals(""))
					vals.put(Constants.LASTCHANGE, c.getLastChange());
	...
	public synchronized void UpdateOldContentsForNews(String Id) {
		...
		try {
			String sql = "Update " + Constants.NEWS_TABLE+ " Set Deleted=1 , Viewd=1 "+ " WHERE " + Constants.ID+ "=" + Id;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public synchronized void UpdateSectionedNews(String Id) {
		...
		try {
			String sql = "Update " + Constants.NEWS_TABLE+ " Set Deleted=1 , Viewd=1"+ " WHERE " + Constants.SECTIONID+ "=" + Id;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public synchronized void deleteOldContentsForNews(int iSectionID) {
		...
		try {
			String sql = "Update " + Constants.NEWS_TABLE+ " Set Deleted=1 , Viewd=1  WHERE " + Constants.SECTIONID+ "=" + iSectionID ;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	private ArrayList<News_Entity>  downloadRecentContentsForNews(String strChangeDate) throws IOException, ParserConfigurationException, SAXException {
...
int iCount=0;
		try {
			String sql = "Select  COUNT(*) FROM " + Constants.NEWS_TABLE + " Where Viewd=0 AND SectionID= "+strsection;
			Cursor crsr =db.rawQuery(sql, null);
			crsr.moveToNext();
	...
	public synchronized void ContentsForAds(String strChangeDate) {
		...
		if (contents != null && contents.size() > 0) {
			if(strChangeDate.equals("0"))
				deletleOldContentsForAds();
	...
	public synchronized void UpdateOldContentsForAds(String Id) {
		...
		try {
			String sql = "Update " + Constants.ADS_TABLE+ " Set Deleted=1 , Viewd=1 WHERE " + Constants.ID+ "=" + Id;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public synchronized void deletleOldContentsForAds() {
		...
		try {
			String sql = "Update  " + Constants.ADS_TABLE+ "Set Deleted=1 , Viewd=1" ;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public synchronized void updateSectionedAds(String Id) {
		...
		try {
			String sql = "Update " + Constants.ADS_TABLE+ " Set Deleted=1 , Viewd=1 WHERE " + Constants.SECTIONID+ "=" + Id;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public synchronized void deleteSectionedAds(String Id) {
		...
		try {
			String sql = "Delete From " + Constants.ADS_TABLE+ " WHERE " + Constants.SECTIONID+ "=" + Id;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public synchronized void deleteOldContentsForAds(int iSectionID) {
		...
		try {
			String sql = "Update " + Constants.ADS_TABLE+ "  Set Deleted=1 , Viewd=1 WHERE " + Constants.SECTIONID+ "=" + iSectionID ;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	private ArrayList<Ads_Entity>  downloadRecentContentsForAds(String strChangeDate) throws IOException, ParserConfigurationException, SAXException {
		return new ContentParser(context.getString(R.string.ads_url) + "" +strChangeDate).getAds();
	}
	
	
	private ArrayList<Ads_Entity>  downloadRecentContentsForAdsByID(String StrID) throws IOException, ParserConfigurationException, SAXException {
...
int iCount=0;
		try {
			String sql = "Select  COUNT(*) FROM " + Constants.ADS_TABLE + " Where Viewd=0 AND SectionID= "+strsection;
			Cursor crsr =db.rawQuery(sql, null);
			crsr.moveToNext();
			iCount= crsr.getInt(0);
		} catch (Exception ex) {
	...
	{ 
		Cursor cur = db.rawQuery("SELECT COUNT(*) FROM  " + Constants.NEWS_FAVORITE_TABLE + " WHERE "+ Constants.ID+" = "+ Id , null);
		cur.moveToFirst(); 
	...
	public synchronized void  deleteFavoriteNews(String id,String section_id) {
		try {
			String sql = "DELETE FROM " + Constants.NEWS_FAVORITE_TABLE +" WHERE " + Constants.ID+ "="+ id+ " AND " + Constants.SECTIONID+ "=" + section_id;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.getMessage();
		}
...
/////////////////////////////////////////////////////////////
	public synchronized void  deleteFavoriteNews(String id) {
		try {
			String sql = "DELETE FROM " + Constants.NEWS_FAVORITE_TABLE +" WHERE " + Constants.ID+ "="+ id;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.getMessage();
		}
	...
	public void updateNumViewd(String Id,String Type,String NumViewd) {
		try {
			   String sql = "DELETE FROM " + Constants.NUMVIED_TABLE + " WHERE " + Constants.ID + "=" + Id +" AND " + Constants.TYPE +" = "+Type;
			    db.execSQL(sql);
		     	ContentValues vals = new ContentValues();
	...
	public synchronized String selectNumViews(String Id,String Type) {
		...
		try {
			...
			Cursor cursor = db.rawQuery("select "+ Constants.NUMVIEWD+" from " + Constants.NUMVIED_TABLE + " where "  + Constants.ID+ "=" + Id+ " AND "+ Constants.TYPE+ "=" + Type, null);
			if (cursor == null)
				   return strNumViews;

				try{
				   if (cursor.moveToFirst()) 
					   strNumViews=cursor.getString(0); 
				}finally { 
				   cursor.close();
				}
			return strNumViews;
	...
	{ 
		Cursor cur = db.rawQuery("SELECT COUNT(*) FROM  " + Constants.ADS_FAVORITE_TABLE + " WHERE "+ Constants.ID+" = "+ Id , null);
		cur.moveToFirst(); 
        int jcount =  cur.getInt(0); 
        cur.close(); 
        return jcount;}
	...
	public synchronized void  deleteFavoriteAds(String id,String section_id) {
		try {
			String sql = "DELETE FROM " + Constants.ADS_FAVORITE_TABLE +" WHERE " + Constants.ID+ "="+ id+ " AND " + Constants.SECTIONID+ "=" + section_id;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.getMessage();
		}
...
/////////////////////////////////////////////////////////////
	public synchronized void  deleteFavoriteAds(String id) {
		try {
			String sql = "DELETE FROM " + Constants.ADS_FAVORITE_TABLE +" WHERE " + Constants.ID+ "="+ id;
			db.execSQL(sql);
		} catch (Exception ex) {
			ex.getMessage();
		}
...
////////////////////////////////////////
	   public boolean updateNewsViewedStatus( String section_id, int newStatus) {
			...
			try {
				String sql = String.format(Locale.US, "UPDATE %s SET %s = %s WHERE (%s = %s) ", Constants.NEWS_TABLE, Constants.VIEWD, newStatus, Constants.SECTIONID,section_id);
				db.execSQL(sql);
				result = true;
			} catch (Exception ex) {
				ex.getMessage();
	   ...
	   public boolean updateAdsViewedStatus( String section_id, int newStatus) {
			boolean result = false;
			try {
				String sql = String.format(Locale.US, "UPDATE %s SET %s = %s WHERE (%s = %s) ", Constants.ADS_TABLE, Constants.VIEWD, newStatus, Constants.SECTIONID,section_id);
				db.execSQL(sql);
				result = true;
			} catch (Exception ex) {
				ex.getMessage();
	   ...
	   public synchronized void ContentsForSponsor(ArrayList<Sponsor_Entity> contents,String strViews) {
		...
		private void saveContentsForSponsor(Sponsor_Entity c,String strViews) {
			try {
					...
					if (c.getEmail() != null && !c.getEmail().equals(""))
						vals.put(Constants.EMAIL, c.getEmail());
					
					if (c.getTel() != null && !c.getTel().equals(""))
						vals.put(Constants.PHONE, c.getTel());
					...
					if (strViews != null && !strViews.equals(""))
						vals.put(Constants.NOOFVIEWS, strViews);
		...
		public synchronized void deleteOldContentsForSponsor() {
		   ...
		   public boolean updateSponsorNoView(String strViews) {
				boolean result = false;
				try {
					String sql = String.format(Locale.US, "UPDATE %s SET %s = %s  ", Constants.SPONSOR_TABLE, Constants.NOOFVIEWS, strViews);
					db.execSQL(sql);
					result = true;
				} catch (Exception ex) {
					ex.getMessage();
