...
public class ContentUpdater {
    ...
    public synchronized void ContentsForEvent(String strChangeDate) {
        try {
            ArrayList<Event_Entity> contents = null;
            contents = downloadRecentContentsForEvent(strChangeDate);

            if (contents != null && contents.size() > 0) {
                for (Event_Entity c : contents)
                    saveContentsForEvent(c);
            }
        } catch (Exception ex) {
            Log.e(TAG, "Error occurred in Contents for Event: " + ex.getMessage());
        }

    }

    private void saveContentsForEvent(Event_Entity event_entity) {
        ContentValues vals = new ContentValues();
        vals.put(Constants.ID, event_entity.getId());

        if (event_entity.getTitle() != null) {
            vals.put(Constants.TITLE, event_entity.getTitle());
        }

        if (event_entity.getDetails() != null) {
            vals.put(Constants.DETAILS, event_entity.getDetails());
        }

        if (event_entity.getLastChange() != null) {
            vals.put(Constants.LASTCHANGE, event_entity.getLastChange());
        }

        if (event_entity.getLastChangeType() != null) {
            vals.put(Constants.LASTCHANGE, event_entity.getLastChange());
        }

        long rid = db.insert(Constants.EVENT_TABLE, null, vals);
        if (rid == -1) {
            String err =
                    "ContentUpdater.saveContents: Unable to insert content: " + event_entity.toString();
            Log.e(TAG, err);
        }
    }
    ...
    private ArrayList<Event_Entity> downloadRecentContentsForEvent(String strChangeDate)
            throws IOException, SAXException, ParserConfigurationException {
    /*return new ContentParser(context.getString(R.string.events_details_url) + "" + strChangeDate)
        .getEventContents();*/
        return new ContentParser(context.getString(R.string.events_details_url))
                .getEventContents();
    }
      ...
      iCount = crsr.getInt(0);*/
            ...
            if (crsr != null) {
                while (!crsr.isAfterLast()) {
                    String endDateString = crsr.getString(0);
                    ...
                    Log.i(
                            "CONTENT UPDATER",
                            "Current Date " + currentDate + " -- EndDate " + endDate + " -- Count " + iCount);
